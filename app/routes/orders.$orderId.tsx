...
function OrderDetailsPage(): JSX.Element {
  ...
  useMemo(() => {
    if (order && order.orderItems) {
      const data: OrderItemWithItemNo[] = order.orderItems.map(
        (orderItem, index) => ({
          ...orderItem,
          itemNo: index + 1,
        }),
    ...
    >
      <div className="flex flex-col p-5">
        <div className="flex gap-2.5">
          <p className="text-xl text-blackrose font-semibold">
            {order.orderNumber}
          </p>
        ...
        <div className="grid grid-cols-4 mt-4 gap-4">
          <div className="w-full col-span-3 bg-white rounded-xl">
            <div className="flex justify-between p-6">
              ...
              <div className="flex flex-col items-end text-sm text-slate-500">
                <p>
                  ยืนยันการสั่งซื้อล่าสุด: {getThaiTimestamp(order.checkoutAt)}
                </p>
                      ...
                      <th
                        ...
                        className={classNames(
                          index === 0 || index === 2
                            ...
                            : "",
                          `w-[${header.getSize()}px]`,
                          "flex-grow flex-shrink-0 h-[3.5rem] px-6 bg-white border-t border-b border-gray-400 justify-start items-center gap-2.5 text-slate-500 text-base text-left font-light font-roboto",
                        )}
                      >
            ...
            <div className="flex flex-row justify-end p-6">
              <div className="flex flex-col w-[25rem] text-sm text-slate-500 font-normal space-y-1">
                <div className="flex justify-between">
                  ...
                  <p>
                    ฿
                    {order.subtotalPrice.toLocaleString("th-TH", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </p>
                ...
                <div className="flex justify-between">
                  ...
                  <p>
                    ฿
                    {order.shipmentPrice.toLocaleString("th-TH", {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </p>
          ...
          <div className="cols-span-1 flex flex-col gap-y-3">
            <div
              className={classNames(
                order.paymentStatus === "PAID" ? "h-[8rem]" : "h-[4rem]",
                "bg-white rounded-xl p-5 space-y-3",
              )}
            >
                ...
                {order.paymentStatus === "PAID" ? (
                  <div className="flex flex-row gap-x-4">
                    <div className="w-12 h-12 rounded-full border bg-[#00427a] border-slate-300 flex items-center justify-center">
                      <img
                        className="w-8 h-8 border-slate-300"
                        src="/images/promptpay.svg"
                        alt="Payment Logo"
                        draggable="false"
                      />
                    </div>
...
function PaymentStatusBadge({ status }: { status: string }): JSX.Element {
  ...
  return (
    <div
      className={classNames(
        isCompleted
          ? "bg-[#c9f6df] text-[#187a41]"
          : "bg-[#f4ebcb] text-yellow-700",
        "flex px-2 rounded-2xl font-light text-xs items-center justify-center",
      )}
    >
