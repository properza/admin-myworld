...
function CustomerOrderDetailsTable (): JSX.Element {
  ...
  const orders = useMemo(() => {
    const orderDetail: CustomerOrderDetail[] = customer.order.map((eachOrder, index) => ({
      itemNo: index + 1,
...
function PaymentStatusBadge ({ isPaid }: { isPaid: boolean }): JSX.Element {
  return (
    <div className="w-[7.5rem] h-9 p-2.5 bg-white border-gray-400 justify-start items-center gap-2.5 inline-flex">
      <div className={cn(isPaid ? "bg-[#cbf4cc]" : "bg-[#f4ebcb]", "w-full px-1.5 py-1 rounded-full items-center gap-1 flex")}>
        <div className="inline-flex justify-start gap-2">
          <CircleFillIcon classNames={isPaid ? "fill-[#1fd831]" : "fill-[#f1bc00]"} />
          <p className="self-center text-black text-xs font-normal font-poppins leading-3">{isPaid ? "ชำระเงินแล้ว" : "รอชำระเงิน" }</p>
        </div>
      </div>
    </div>
  )
...
function OrderDetailButton ({ orderId }: { orderId: string }): JSX.Element {
  return (
    <div className="h-6 px-1.5 py-0.5 bg-white rounded border border-sky-400 justify-start items-center gap-1 inline-flex">
      <Link to={`/orders/${orderId}`} className="text-sky-400 text-xs font-normal font-roboto">ดูรายละเอียด</Link>
