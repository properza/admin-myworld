...
function CustomerTradeDetailsTable(): JSX.Element {
	...
	const trades = useMemo(() => {
		const tradeDetail: CustomerTradeDetail[] = customer.trade.map(
			(eachTrade, index) => ({
				itemNo: index + 1,
				...
				shipmentStatus: [eachTrade.shipment_status, "LOL"][
...
function ProductImageBox({ src }: { src: string }): JSX.Element {
	return (
		<img
			className="w-[27px] h-[27px] border border-gray-400"
			src={src}
			alt="Merchandise"
			draggable="false"
		/>
...
function ShippingStatusBadge({
	isDelivered,
}: { isDelivered: boolean }): JSX.Element {
	return (
