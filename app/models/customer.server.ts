...
export async function getCustomers (accessToken: string, params?: Partial<GetCustomerParams>): Promise<CustomerResponse> {
  const endpointURL = constructURL(config.api.baseUrl, "/customers/administrator");
...
export async function getCustomer (accessToken: string, userId: string): Promise<Customer> {
  const endpointURL = constructURL(config.api.baseUrl, `/customers/administrator/${userId}`);
