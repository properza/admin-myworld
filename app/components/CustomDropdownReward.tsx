...
const CustomDropdownReward: React.FC<CustomDropdownRewardProps> = ({ options, defaultOption }) => {
  ...
  return (
   <div className="flex relative" ref={dropdownRef}>
    <div className="h-6 px-1.5 py-0.5 bg-white rounded border border-sky-400 cursor-pointer" style={{borderColor :  selectedOption.color , color :   selectedOption.color }}>
      <div className="flex justify-start items-center gap-1" onClick={toggleDropdown}>
        <p className="text-xs font-normal font-roboto">{selectedOption.title}</p>  
        <img src="/images/chevron-down.svg" alt="More options" className="w-2 h-2 justify-center items-center" draggable="false"  style={{fill: selectedOption.color}} />
      </div>
      {isOpen && (
